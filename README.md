...
### 特徴 2: 生成 AI による技術解説とファクト抽出

-   **Gemini 2.5 Flash の活用:** Google の Gemini API を利用して解析を行います。
-   **厳格なシステムプロンプト (`backend/src/prompt/system_prompt.py`):**
    -   AI に対し、**「事実」「推論」「創造的応用」の 3 層**を明確に区別するよう指示しています。
    -   記述には必ず `[段落: 0023]` や `[図: 1]` といった**出典（根拠）の明記**を強制しており、信頼性を担保する設計になっています。
